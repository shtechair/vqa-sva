"""
Reference: https://github.com/Cadene/skip-thoughts.torch/tree/master/pytorch
Difference from PyTorch official version:
  1. built from basic pytorch operators instead of RNN cells
  2. using sequential dropout where the dropout mask is shared in all steps
"""
import torch
import numpy as np
...
class EmbeddingDropout(nn.Module):
    """
    In each sequence, selecting which character token to drop. If selected, drop all of them.
    Did not inherent from module, maybe it is because it does not need gradient?
    """
    ...
    def forward(self, input):
        ...
        if self.p > 0 and self.training:
            ...
            if dim == 1:
                # transform back into the initial shape
                input = input.view(-1)
...
class SequentialDropout(nn.Module):
    ...
    def forward(self, input):
        if self.p > 0 and self.training:
            if self.restart:
                ...
                self.noise = self.noise.expand_as(input) # why this line?
                self.restart = False
    ...
    def backward(self, grad_output):
        # reset the state at every backward
